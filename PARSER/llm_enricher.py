...
load_dotenv(".env")        # extra explicit safety
...
OPENAI_MODEL = os.getenv("CLAUDE_MODEL", os.getenv("OPENAI_MODEL", "claude-opus-4-5"))
...
def _get_path(obj: Any, path: str) -> Any:
    ...
    for part in parts:
        ...
        for idx_s in idxs:
            idx = int(idx_s)
            if isinstance(cur, list) and 0 <= idx < Slen(cur):
                cur = cur[idx]
            else:
                return None
    return cur
