...
generated XOSC file's XML with the engineer's chosen overrides plus# parameter_overrides.py - Field-driven parameter override logic
"""
------------------------------------------------------------
Responsible for: Determining which protocol-extracted fields are safe to
show and edit for a given scenario, and deterministically patching a
...
_VERIFIED_CCR_SPEED_TABLE: Dict[str, Dict[str, Any]] = {
    ...
    # CCRb: the hero AEB trigger is 12.0; the lead vehicle's own brake distance is
    # the headway (default 25.0) — see thesis Section 7.2.3.
    "CCRb": {"ego_options_kph": [50], "trigger_m": {50: 12.0}, "target_kph": 50},
}

# Canonical, always-enforced AEB trigger values for scenarios outside the
# CCR family. These are fixed simulator-validated values rather than values
# derived from extracted parameters, because no closing-distance safety check
# exists for these geometries (thesis Section 7.5).
_CANONICAL_TRIGGER_TABLE: Dict[str, float] = {
    ...
    "CCCscp": 48.0,   # fixed simulator-validated value (thesis Section 4.9.1)
...
def apply_canonical_corrections(xosc_code: str, scenario: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    Forces the hero's AEB trigger to the fixed validated value for
    ...
    if scenario_code == "CCCscp":
        warnings.append(
            "CCCscp: trigger fixed at 48.0 (simulator-validated value). No closing-distance "
            "safety check exists for crossing geometries — see thesis Section 7.5."
        )
